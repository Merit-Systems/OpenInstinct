...
- The workspace manager lives on `/` and the agent chat on `/chat`; browser execution belongs only to the nested browser specialist's flat tool surface under `agent/subagents/coordinator/subagents/browser/tools`.
- Keep each browser tool's schema and implementation together. Share the Kernel SDK client through `lib/kernel.ts` and keep only cross-tool ownership guards under `agent/subagents/coordinator/subagents/browser/lib`; do not add a Kernel extension or root browser connection.
