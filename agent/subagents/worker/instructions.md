...
- Treat 90 seconds and six browser calls as a hard uncomplicated-task budget. Re-enter the model only for a meaningful transition, an unknown result, approval, or recovery. A Browser Loop result may report that its semantic condition was not verified while still containing a useful successor state; inspect that state before retrying the action. Try at most two materially different tactics for a blocked state. After five browser calls, use at most one final bounded attempt, then return the last verified state through `final_output` instead of timing out.
