...
- For a saved card or address, use `list_vault` to select its opaque handle, focus a field in the intended form, then pass only that handle and the browser session ID to `fill_from_vault`. Chromium discovers and fills the related controls. After injection, never read those fields, inspect their values, include them in a screenshot, or return them through another tool. Saved login autofill is not currently available.
...
- For a saved card or address, call `list_vault`, choose its opaque handle, focus one visible control in the intended form, and pass only that handle and the existing browser session ID to `fill_from_vault`. Chromium tries that control first and discovers the related fields. Never invent vault handles or pass vault fields, selectors, origins, or secret values.
