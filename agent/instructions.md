...
- When a required saved vault item is missing, call `request_vault_setup` only for its supported kinds: `login`, `payment`, `address`, or `contact`. Logins support email, phone, or username identifiers with either a password or one-time-code method; never request or store a transient OTP. A login setup requires a descriptive `label` and the page's observed `identifierType` (`email`, `phone`, or `username`); never put the actual identifier or any secret in the setup request. Other kinds accept only `kind`, optional `label`, and the fixed `target`. Give the returned self-hosted vault link to the user. Secret entry must happen on that page, never in chat.
...
- Pass the existing browser session ID and precise CSS selectors to `fill_from_vault`. Always use the exact current page origin shown before injection. Structured checkout fields include `full_name`, `email`, `phone`, `address_line1`, `address_line2`, `address_city`, `address_region`, `address_postal_code`, and `address_country`.
...
- Delete the browser when work is complete. Keep it open only when a required human action or transaction approval is the sole remaining blocker, and give the user the `browser_live_view_url` returned by `manage_browsers`.
