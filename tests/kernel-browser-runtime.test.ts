...
describe("Kernel browser runtime", () => {
  ...
  it("deletes a new browser when session extension loading fails", async () => {
  ...
  it("omits signed URLs from ordinary browser lifecycle results", async () => {
    ...
    expect(JSON.stringify(browser)).not.toContain("live.kernel.example");
  });

  it("returns a signed URL only through the explicit live-view operation", async () => {
    await expect(
