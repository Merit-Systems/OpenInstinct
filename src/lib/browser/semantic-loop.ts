...
async function resourcesFor(sessionId: string, signal?: AbortSignal) {
  ...
  const resources = new LoopExecutionResources({
    browser,
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- Browser Loop pins an older nominal Kernel SDK type, while this app supplies the API-compatible shared client required by the repository contract.
